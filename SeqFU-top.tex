...
\end{defn}
\begin{defn}
    Let \((A, \prec)\) be a linearly ordered set. We say that  \(B \subseteq A\) is unbounded in \(A\) if there does not exist \(y \in A\) such that \(x \prec y\) for all \(x \in B\). We say that \(C \subseteq A\) is cofinal (or dominating) in \(A\) if for all \(x \in A\) there exists \(z \in C\) such that \(x \prec x\).
\end{defn}
\begin{defn}
    \leavevmode
    \begin{itemize}
...
\begin{defn}
    Define an order on \(\P(X)\) for some set \(X\). We say \(A \asubseteq B\) if \(B\) contains all but finitely many points of \(A\). If \(\F \subseteq \P(X)\) is a collection of (infinite) sets, then \(A\) is a pseudo-intersection of \(\F\) if \(A \asubseteq F\) for all \(F \in \F\).
\end{defn}
\begin{defn}
    We call \(\T \subseteq [\omega]^{\omega}\) a tower if \(\T\) is well ordered by \(\asubseteq\) and has no infinite intersection.
\end{defn}
\begin{lem}
    If \(\A = \{A_n \: : \: n \in \omega\} \subseteq [\omega]^{\omega}\) such that for all \(n \in \omega\)
    \[
        A_0 \asupseteq  A_1 \asupseteq \dots \asupseteq  A_n
    \]
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \item If \(\alpha\) is a limite ordinal then define \(A_{\alpha}\) such that \(A_{\alpha} \asubseteq A_{\beta}\) for all \(\beta < \alpha\)
    \end{enumerate}
...
\subsection{Finite Frechet-Urysohn spaces}

\begin{defn}
    A network of a space \(X\) is a collection \(\F \subseteq \P(X)\) such that for all \(x \in X\) and for all nbhds \(U_x\) there is \(F \in \F\) such that \(x \in F \subseteq U_x\).
\end{defn}

\begin{defn}
    \(X\) is Fréchet-Urysohn Finite (FUF) if whenever \(\F\) is a network in \(X\) there exists \(\{F_n \: : \: n \in \omega\} \subseteq \F\) such that for all nbhds \(U\) there exists \(n \in \omega\) such that \(x \in F_m \subseteq U\) for all \(m > n\).
\end{defn}
