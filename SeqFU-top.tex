...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Winning strategy in proximal game implies Frechet
...
\end{proof}
%\begin{defn}
%    Let \(x \in X\) and let \(\tau: [X]^{<\omega} \times \O_x(X) %\to X\) be a strategy for player 2 at \(x\). Say that \(\F_%{\tau} = \langle F_n  \: : \: n \in \omega\rangle\) is a \%(\tau\)-chain if for all \(n \in \omega\) we have \(F_n \subset %F_{n + 1}\) and \(F_{n + 1} = F_n \cup \{\tau(F_n, U)\}\) for %some \(U \in \O_x\).
%\end{defn}
\begin{prop}
...
\begin{exercise}
    If player 1 (player 2) has a winning strategy (no winning strategy) in proximal game on \(X\) then \(X\) is Fréchet.
\end{exercise}
\begin{prop}
    The Proximal game is equivalent to the Gruenhage game.
\end{prop}
 Using topological groups to de
