...
\begin{proof}
    ...
    On step 0, take \(A_0 \subset [\omega]^{<\omega}\). If it's a network for \(\F_0\) then we find \(B_0 \subset A_0\) such that \(B_0 \to \F_0\), which exists since \(\omega\) is first countable. Otherwise, if \(A_0\) isn't a network, we proceed to the next step. 
    ...
    We continue step \(\alpha\) by taking \(A_{\alpha} \subset [\omega]^{\omega}\). If it's a network for \(\F_{\alpha}\), find another sequence \(B_\alpha \subset A_{\alpha}\) that converges with respect to \(\F_{\alpha}\), otherwise proceed to the next step.
...
%\section{Musings}
%The point of this section is to keep track of thoughts relating to problems whose solutions are maybe not be clear or known. 


%\subsection{FUF implies SP}
