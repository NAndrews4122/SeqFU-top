...
\begin{defn}
    \(\A \subset \P(X)\) is a network if for all \(x \in X\) and for all nhds \(U \ni x\) there is \(A \in \A\) such that \(x \in A \subset U\). If \(\F\) is a filter, then we might also refer to a filterbase for \(\F\) as a network. 
\end{defn}
\begin{rem}
...
\begin{prop}
    \leavevmode
    \begin{enumerate}
        \item[(1)] (Sipacheva) If \(\F\) is FUF then \(G\) is Frechet
